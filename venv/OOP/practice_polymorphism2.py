...
간단한 주행 시뮬레이터 설명
주행 시뮬레이터
    1. 여러 가지 교통 수단들(일반 자동차, 스포츠카, 자전거 등)을 가질 수 있음
    2. 갖고 있는 교통 수단들의 주행을 동시에 시작/정지 시킬 수 있음
    3. 갖고 있는 교통 수단들의 현재 속도를 문자열 메시지로 볼 수 잇음
"""
...
class DrivingSimulator:
    ...
    def start_all_vehicles(self):
        ...
        [vehicle.start() for vehicle in self.vehicles]

    def stop_all_vehicles(self):
        """모든 교통 수단을 주행 정지시킨다"""
        print("모든 교통 수단을 주행 정지시킵니다!\n")

        [vehicle.stop() for vehicle in self.vehicles]  # list comprehension
